...
<section id="edition_block">
    <div class="content_wrapper">
        ...
        <div class:opened={isOpen} class="panel_style" id="dropdown_wrapper" bind:this={dropdownDiv}>
            <ul>
            {#each editions as e}
                <li><button class:checked={e.name === championshipEdition.edition} type="button" onclick={() => championshipEdition.define(e.name, e.id, e.brackets, e.prefetched)}>{e.name}</button></li>
            {/each}
            </ul>
        </div>
    </div>
